...
const API_URL = "https://it25u6dfgh.execute-api.us-east-1.amazonaws.com/default/fn-upload";
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      let resp = await get(API_URL+"?file="+file.name+"&cmd=put");
      ...
      console.log("get signed url for download");
      resp = await get(API_URL+"?file="+responseString.file+"&cmd=get");
      console.log("resp:", resp);
      setResponseString(resp);
 
      setStatus(`url: ${resp.file}`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      ...
      <a href={responseString.signedUrl} target="_blank"
      style={{backgroundColor:'#FFF'}}>Download {responseString.file}</a>
